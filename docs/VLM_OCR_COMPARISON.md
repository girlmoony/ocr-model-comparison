# PaddleOCR / PaddleOCR-VL / DeepSeek-OCR (v1 & v2) / baidu Unlimited-OCR — Local GPU Benchmark Report

Conducted: 2026-07-06 to 07 / Environment: RTX 4060 Ti (8GB), Windows 11

The 5 images in `samples/ocr/` (two-page spreads of a vocabulary book photographed with a smartphone, 4032×3024) were OCR'd with all five systems and scored against `samples/ocr-expected/*.json`.

## Run Configuration

| | PaddleOCR | PaddleOCR-VL-1.6 | DeepSeek-OCR (v1) | DeepSeek-OCR-2 | baidu/Unlimited-OCR |
|---|---|---|---|---|---|
| Runtime | paddlepaddle-gpu 3.2.1 (cu126) | paddleocr 3.7.0 `PaddleOCRVL` | **Ollama 0.31.1** (`deepseek-ocr:latest`) | transformers (venv) | transformers (venv) |
| Model / precision | PP-OCRv6 medium det+rec, `lang=japan` | ~1B / BF16 (ERNIE-4.5 base) | 3.3B / F16 | ~3.4B / BF16 | ~3.3B / BF16 |
| torch / transformers | — (Paddle runtime) | — (Paddle runtime) | — (Ollama runtime) | 2.6.0+cu126 / 4.46.3 | 2.10.0+cu128 / 4.57.1 |
| Prompt | — (det+rec pipeline) | — (doc-parsing pipeline, `pipeline_version="v1.6"`) | `Convert the document to markdown.` | `<image>\n<|grounding|>Convert the document to markdown. ` | `<image>document parsing.` |
| Resolution settings | default (max side 4000) | default | Ollama defaults (context 8192) | base_size=1024, image_size=768, crop_mode=True | base_size=1024, image_size=640, crop_mode=True |
| Peak VRAM | minimal | ~1B model (fits easily) | ~6.7GB (model size) | 8.16 GB | 8.09 GB |
| Time per page | **1.2–2.0 s** | 76–141 s | 9–20 s | 31–130 s | 32–52 s (968 s when looping) |
...
### PaddleOCR-VL-1.6 (doc-parsing pipeline)

| sample | word-recall | meaning | exampleEn | exampleJa |
|---|---|---|---|---|
| IMG_4122 | 8/8 | 7/8 | 8/8 | 7/8 |
| IMG_4123 | 8/8 | 7/8 | 8/8 | 1/8 |
| IMG_4124 | 8/8 | 8/8 | 8/8 | 6/8 |
| IMG_4125 | 8/8 | 6/8 | 8/8 | 4/8 |
| section-11 | 8/8 | 8/8 | 8/8 | 2/8 |
| **Total** | **40/40** | **36/40** | **40/40** | **20/40** |

- **The overall accuracy winner**: best meanings (36/40), perfect English examples (40/40), tied-best Japanese translations (20/40), zero loops, order fully preserved. The layout-analysis stage reconstructs reading order, fixing exactly what breaks classic PaddleOCR on this multi-column layout.
- Weak spots: on IMG_4123 and section-11 the table reconstruction misassigned cells — e.g. on IMG_4123 the Japanese-translation column collapsed into a single repeated sentence, so exampleJa scored 1/8 there despite the English column surviving intact.
- Slowest system in this test (76–141 s per page, plus an 85 s pipeline load) with the default Paddle backend. The official vLLM server backend is the documented path to faster inference.
- This is the same model family as the Hugging Face Space "PaddleOCR-VL-1.6 Online Demo" — results there are consistent with what this local run produces.
...
| Metric | Tesseract only | PaddleOCR | PaddleOCR-VL-1.6 | DeepSeek-OCR (v1/Ollama) | DeepSeek-OCR-2 | Unlimited-OCR |
|---|---|---|---|---|---|---|
| Headwords (word-recall) | 21/40 | **40/40** | **40/40** | **40/40** | **40/40** | 39/40 |
| Japanese meanings | 0% | 28/40 | **36/40** | 8/40 | 28/40 | 27/40 |
| English examples | 0% | 2/40 | **40/40** | 36/40 | 38/40 | 36/40 |
| Japanese translations | 0% | 2/40 | **20/40** | 3/40 | 15/40 | **20/40** |
| Repetition loops | — | **0/5** | **0/5** | **0/5** | 1/5 | 1/5 |
| Time per page | — | **1.2–2.0 s** | 76–141 s | 9–20 s | 31–130 s | 32–52 s |

## Observations

0. **PaddleOCR-VL-1.6 is the overall accuracy winner** (meanings 36/40, English examples 40/40, translations 20/40 tied-best, zero loops). Its layout analysis solves the reading-order problem that cripples classic PaddleOCR, at the cost of being the slowest system here (76–141 s per page on the default backend) and occasionally misassigning table cells on complex spreads.
0-b. **Classic PaddleOCR is in a different speed class** (1.2–2.0 s per page) with strong character recognition (headwords 40/40, meanings 28/40), but it has no layout understanding: multi-line sentences in the multi-column layout come out as interleaved fragments, so example-sentence fields are unusable without box-coordinate post-processing.
...
- **Best accuracy for every field (verbatim extraction)**: **PaddleOCR-VL-1.6** — meanings 36/40, English examples 40/40, translations 20/40, no loops. Accept the slow default backend (76–141 s per page) or deploy the official vLLM server backend; watch for occasional table-cell misassignment.
- **If VL is too slow and you still need verbatim fields**: baidu/Unlimited-OCR or DeepSeek-OCR-2 (Unlimited recommended by a small margin — better capture rate on the Japanese-translation column and more consistent speed). Cap `max_length=8192` and add a repetition-detection retry guard.
...
- `results/paddleocr-vl/<name>/<name>.md` + `<name>_res.json` (PaddleOCR-VL structured output)
...
```powershell
# same paddle-env, plus:
uv pip install --python paddle-env\Scripts\python.exe -U "paddleocr[doc-parser]>=3.6.0"
paddle-env\Scripts\python.exe scripts\paddle_vl_ocr.py samples\ocr results\paddleocr-vl v1.6
```
